...
test('project screen uses personalized project names and strengths',()=>{
  ...
  const b2=loadScript('js/app.js',{window:{InterviewOSData:d2,InterviewStore:store,InterviewScheduler:sched,InterviewSimulations:{},InterviewDiagrams:{render:()=>'<svg></svg>',renderVisual:v=>'<svg data-visual=+(v&&(v.type||v.named)||)+></svg>',has:()=>true}}});
...
test('mock interview reveals category and follow-up questions',()=>{
  ...
  const b4=loadScript('js/app.js',{window:{InterviewOSData:d4,InterviewStore:store,InterviewScheduler:sched,InterviewSimulations:{},InterviewDiagrams:{render:()=>'<svg></svg>',renderVisual:v=>'<svg data-visual=+(v&&(v.type||v.named)||)+></svg>',has:()=>true}}});
...
test('roadmap shows priority requirement coverage when requirement mappings exist',()=>{
  ...
  const b3=loadScript('js/app.js',{window:{InterviewOSData:d3,InterviewStore:store,InterviewScheduler:sched,InterviewSimulations:{MessagingSim:{run:()=>({capacity:1,backlog:0,utilizationPct:1,replay:'Strong',ordering:'x',ack:'x'})}},InterviewDiagrams:{render:()=>'<svg></svg>',renderVisual:v=>'<svg data-visual=+(v&&(v.type||v.named)||)+></svg>',has:()=>true}}});
